...
RETURNS SETOF RECORD LANGUAGE SQL AS $$
    ...
    FROM (
        ...
        SELECT log_time AS start_time, log_time + (time_interval_minutes::STRING || ' MINUTES')::INTERVAL AS end_time FROM (
...
RETURNS SETOF RECORD LANGUAGE SQL AS $$
    ...
    LEFT JOIN (
        SELECT us.host_ip, COUNT(us.session_id) AS session_count, SUM(us.resource_size) AS resource_load
